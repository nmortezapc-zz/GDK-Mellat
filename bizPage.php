        ...
        <div class="page-top-info container" style="line-height:auto">
            <div class="row">
                <div class="col-md-4 col-xs-12 col-sm-3 pull-left" style="text-align:left;">
                    <a href="<?=$siteurl;?>/@<?=$primkey;?>/photos/گالری_عکس_<?=str_replace(" ", "_",$biz->name);?>" class="btn3d btn-danger btn-md pb5"><i class="fa fa-photo ml10"></i> تصاویر</a>
                </div>
                <div class="col-md-8 col-xs-12" style="text-align:right;">
                    <a href="<?=$siteurl;?>/@<?=$primkey;?>" title="<?=$biz->name;?>"><h3 class="white biz-title"><?=$biz->name;?></h3></a>
                    <?
                    ...
                    $ctname = locdata($biz->city)->local_name;
                    ?>
                    <span class="rating-input gold mr10 lgsize" data-stars="<?=ceil($stars);?>-5<?=$haveHalf;?>"></span> <?=$biz->review_count;?> <span class="small">رای (مجموع : <?=$biz->review_sum == "" ? "0" : $biz->review_sum ;?>)</span><br/>
                    <i class="fa fa-map-marker ml10"></i><a href="<?=$siteurl;?>/search/city=<?=$ctname;?>/" class="white"><?=$ctname;?></a> - <a href="<?=$siteurl;?>/search/city=<?=$ctname;?>-subct=<?=$biz->subcity;?>" class="white"><?=citydata($biz->subcity)->name;?></a>
                </div>
            </div>
            <br/>
        </div>
    ...
    <div class="container mb15">
        <div class="row">
            <div class="col-md-6">
                ...
                <img src="http://s5.citygramcdn.ir/media/grpMwVNgWbeiZ05YnaPT/pic150.png" width="150px" class="img-responsive pull-left mr20" alt="photoUplaod" />
            ...
            <div class="col-md-6">
                <form id="uploadimage" action="" method="post" enctype="multipart/form-data">
                    ...
                    <div id="image_preview" style="background:#fff !important;text-align:center">
                        <img id="previewing" src="http://citygramcdn.ir/bphoto/<?=urlencode(imageEncrypt('usrUpload/text2.png:" ":::1'));?>/220.ctg" />
                    </div>
                    <hr id="line">
                    <div id="selectImage">
                        <input type="hidden" value="<?=encryptIt("uploadFile_".random_string('5'));?>" name="key">
                        ...
                        <input type="hidden" name="kind" value="<? echo encryptIt("userUpload_".random_string('6'));?>" />
                        <label>تصویر خودتون رو انتخاب کنید : </label>
                        <input type="file" name="file" class="file_Upload" required />
                        <button type="submit" class="btn btn-primary sbtn" name="submitBtn">شروع آپلود</button>
                    </div>
...
<? }else if($action == "photos"){
    ...
    <section class="page-top basic">
        <div class="page-top-info container" style="line-height:auto">
            <div class="row">
                <div class="col-md-4 col-xs-12 col-sm-3 pull-left" style="text-align:left;">
                    <a href="<?=$siteurl;?>/@<?=$primkey;?>/addPic" class="btn3d btn-danger btn-md pb5"><i class="fa fa-photo ml10"></i> افزودن عکس</a>
                </div>
                <div class="col-md-8 col-xs-12 profTitle" style="text-align:right;">
                    <a href="<?=$siteurl;?>/@<?=$primkey;?>" class="white"><h3 class="white biz-title"><?=$biz->name;?></h3></a>
                    <?
                    if($biz->review_count !== "0" ){
                        $stars = $biz->review_sum/$biz->review_count;
                        $haveHalf = is_float($stars) ? "half" : "";
    ...
    <div class="container mb15">
        <style type="text/css">
            .hide-bullets{list-style:none;padding-right:0}
            .hide-bullets li{float:right;padding-right:0}
            .thumbnail{padding:0}
            .carousel-inner>.item>img,.carousel-inner>.item>a>img{width:100%}
        </style>
        <div class="row">
            <div class="col-md-12 col-xs-12">
                <span class="pull-left">نمایش تصاویر <?=(++$start);?> تا <?=$count < $ftspp ? $count : $ftspp*$page;?> از مجموع <?=$count;?> </span>
                            ...
                            foreach($pics as $pic){?>
                                ...
                                <li class="col-sm-3">
                                    <a class="thumbnail" id="carousel-selector-<?=$picTumbId ;?>">
                                        <img alt="<?=$biz->name;?>" src="http://citygramcdn.ir/bphoto/<?=urlencode(imageEncrypt($pic['address'].': " سيتي گرام " :1: 10: :1'));?>/122.ctg" style="width:122px;height:122px">
                                    </a>
                                </li>

                                <?
                                $tumbImage = urlencode(imageEncrypt($pic['address'].': " سيتي گرام " : 0: 17: :1'));
                        ...
                        <div class="row">
                            <div class="col-sm-12" id="carousel-bounding-box">
                                <div class="carousel slide" id="myCarousel">
                                    ...
                                    <a class="left carousel-control" style="left:15px !important" href="#myCarousel" role="button" data-slide="prev">
                                        <span class="fa fa-arrow-circle-left gray" style="font-size:40px !important"></span>
                                    </a>
                                    <a class="right carousel-control" style="right:8px !important" href="#myCarousel" role="button" data-slide="next">
                ...
                <hr/>
                <span class="pull-right mb10"><div class="ftpagenav"><?=$firstpage.' '.$pervpage.' '.$paged_others.' '.$nextpage.' '.$lastpage;?></div></span>
            </div>
        </div>
    </div>
    <br/>
<? }else if($action == "navigate"){
    ...
    <style>
        ...
        #instructions{padding-right:5px;list-style-type:none}
        .mapInfoWindow{min-width:250px;text-align:right;margin:0 !important;padding:0 !important}
        .mapInfoWindow{}
    </style>
    <div role="main" class="main">
    <section class="page-top basic">
            ...
            <div class="col-md-4">
                ...
                <div class="mt15">
                    ...
                    <select id="CurrentLoc" class="noselect" style="width:50%">
                        ...
                        if($logged){
                            ...
                            foreach($userPlaces as $userPlace ){ ?>
                                <option data-lat="<?=$userPlace['lng'];?>" data-lng="<?=$userPlace['lat'];?>" value="<?=$userPlace['id'];?>"><?=$userPlace['title'];?></option>
                            <? }
                        }?>
                        <option value="getCurrent">محل فعلی شما</option>
                    </select><?if($logged){?>
                        <a href="<?=$siteurl;?>/usercp/addNewPlace/" class="mr10 inline"><i class="fa fa-map-marker"></i> تعریف مکان</a>
                    <? } ?>
                    <hr/>
                    ...
                    <form id="navigate">
                        <div class="mt15">
                            راهبری با :
                            <label class="ml5"><input type="radio" name="kindofNavigate" value="driving" checked /> <i class="fa fa-car lgsize mr5"></i></label>
                            <label class="ml5"><input type="radio" name="kindofNavigate" value="bicycling" /> <i class="fa fa-bicycle llgsize mr5"></i></label>
                            <label class="ml5"><input type="radio" name="kindofNavigate" value="transit" /> <i class="fa fa-subway llgsize mr5"></i></label>
                            <label class="ml5"><input type="radio" name="kindofNavigate" value="walking" /> <i class="fa fa-male llgsize mr5"></i></label>
                        </div>
...
<? }else{
    ...
    $pkeywords = $biz->name.",سیتی گرام,صفحه کسب و کارهای سیتی گرام";
        ...
        <div class="page-top-info container" style="height:260px;line-height:auto">
            <div class="row">
                    ...
                    <div>
                        <? if($logged){
                            if($biz->owner == $user->user_id){?>
                                ...
                                <a href="<?=$siteurl;?>/BusinessCP/stats-<?=$primkey;?>" class="btn3d btn-danger btn-md pb5"><i class="fa fa-briefcase ml10"></i> پنل مدیریت</a>
                                <a href="<?=$siteurl;?>/BusinessCP/photos-<?=$primkey;?>" rel="tooltip" title="مدیریت تصاویر" data-toggle="tooltip" date-placement="bottom" class="btn3d btn-info btn-sm"><i class="fa fa-photo"></i></a>
                                <a href="<?=$siteurl;?>/BusinessCP/stats-<?=$primkey;?>" rel="tooltip" title="آمار کلی" data-toggle="tooltip" date-placement="bottom" class="btn3d btn-info btn-sm"><i class="fa fa-line-chart"></i></a>
                                <a href="<?=$siteurl;?>/BusinessCP/setting-<?=$primkey;?>" rel="tooltip" title="تنظیمات" data-toggle="tooltip" date-placement="bottom" class="btn3d btn-info btn-sm"><i class="fa fa-cog"></i></a>
                            ...
                            <? }else{
                                ...
                                <a href="#onlineReservation" data-toggle="modal" class="btn3d btn-danger btn-md pb5"><i class="fa fa-shopping-bag ml10"></i> رزرو کردن</a>
                                <a href="<?=$siteurl;?>/@<?=$primkey;?>/photos/گالری_عکس_<?=str_replace(" ", "_",$biz->name);?>" rel="tooltip" title="گالری عکس" data-toggle="tooltip" date-placement="bottom" class="btn3d btn-info btn-sm"><i class="fa fa-photo"></i> تصاویر</a>
                                <?=$FollowButton;?>
                                <?=$LikeButton;?>
                            <? }
                        }else{?>
                            <a href="<?=$siteurl;?>/@<?=$primkey;?>/photos/گالری_عکس_<?=str_replace(" ", "_",$biz->name);?>" rel="tooltip" title="گالری عکس" data-toggle="tooltip" date-placement="bottom" class="btn3d btn-danger btn-md"><i class="fa fa-photo"></i> تصاویر</a>
                        <?}?>
                ...
                <div class="col-md-8 col-xs-12 col-md-9 pull-right profTitle" style="text-align:right;">
                    ...
                    ?>
                    <span class="rating-input gold mr10 lgsize" data-stars="<?=ceil($stars);?>-5<?=$haveHalf;?>"></span> <?=$biz->review_count;?> <span class="small">رای (مجموع : <?=$biz->review_sum == "" ? "0" : $biz->review_sum ;?>)</span><br/>
                    <a href="" class="white"><i class="fa fa-map-marker ml10"></i><a href="<?=$siteurl;?>/search/city=<?=$ctname;?>/" class="white"><?=$ctname;?></a> - <a href="<?=$siteurl;?>/search/city=<?=$ctname;?>-subct=<?=$biz->subcity;?>" class="white"><?=citydata($biz->subcity)->name;?></a>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    ...
                    <!-- Map part start -->
                    <div class="col-md-4  col-xs-12 pull-right" style="border-radius:6px;background:#fff;color:#555;margin-top:10px;padding:5px;text-align:right;">
                        <img src="https://maps.googleapis.com/maps/api/staticmap?language=fa&center=<?=$biz->lat;?>,<?=$biz->lng;?>&size=400x200&sensor=true&zoom=16&markers=color:red|<?=$biz->lat;?>,<?=$biz->lng;?>&scale=1" id="pagepic" class="rad5" style="width:100%;display:block;height:auto"  />
                        <span class="ssmall"><?=$biz->address;?></span>
                        <div class="row">
                            <div class="col-md-6 small">
                                <a href="<?=$siteurl;?>/@<?=$primkey;?>/navigate/راهبری_به_<?=str_replace(" ", "_",$biz->name);?>"><i class="fa fa-map-marker"></i> راهبری به این مکان</a>
                            </div>
                    ...
                    <div class="col-md-6 col-xs-12">
                        ...
                        if($query_pcms->rowCount() > 0){?>
                            <div class="carousel slide" data-ride="carousel" id="quote-carousel">
                                    ...
                                    foreach($bizComments as $bizComment){
                                        ...
                                        <div class="item <?= $cmID == 1 ? "active" : "";?>">
                                            <blockquote>
                                                <div class="row">
                                                    <div class="col-sm-9">
                                                        <div class="avgsize"><?=html_entity_decode($bizComment['comment']);?></div>
                                                        <a class="green lgsize mt5" href="<?=$siteurl;?>/profile/<?=$bizComment['user'];?>/<?=str_replace(" ", "_",$cmUser->realname);?>" target="_blank"><small><?=$cmUser->realname;?></small></a>
                                                    </div>
                                                    <div class="col-sm-3 text-center">
                                                        <img class="img-circle" src="http://citygramcdn.ir/avatar/<?=urlencode(imageEncrypt($cmavatar));?>/100.ctg" style="width: 100px;height:100px;" />
                                                    </div>
                                                </div>
                                            </blockquote>
                                        </div>
                                ...
                                <a data-slide="prev" href="#quote-carousel" class="left carousel-control"  style="padding-top:12% !important"><i class="fa fa-chevron-left"></i></a>
                                <a data-slide="next" href="#quote-carousel" class="right carousel-control"  style="padding-top:12% !important"><i class="fa fa-chevron-right"></i></a>
                            </div>
                        <?}else{
                            // بدون نظر
                        }?>
            ...
            <div class="col-md-4 col-xs-12">
                <div class="prof-biz-reserve">
                    ...
                    <div class="biz-content">
                        ...
                        if($tommorrowProgram == 0 && $todayProgram == 0 )
                            ...
                            $closed = "";
                            if($todayWeekAndHour[1] >= $todayProgram)$closed = "بودیم <span class='ssmall red'>الان بسته ایم .</span>";
                            else $closed = "هستیم .";
                            $todayTime = "امروز تا ".$todayProgram." ".toDateName($todayProgram)." باز ".$closed  ;
                        }
                        ?>

                        <span class="small" ><?=$todayTime;?></span>
                        <div class="mt10" style="text-align:center">
                            <? if($ReserveActivePerm){?><a href="#onlineReservation" data-toggle="modal" class="btn btn-dodored"><i class="fa fa-shopping-bag"></i> رزرو آنلاین</a><?}?>
                            <a href="<?=$siteurl;?>/@<?=$primkey;?>/navigate/راهبری_به_<?=$biz->name;?>" class="btn btn-dodored pb5"><i class="fa fa-map-marker"></i> یافتن روی نقشه</a>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="panel-body" >
                            <div class="llgsize red mb10" style="border-bottom:1px dashed #f99595;padding-bottom:10px">
                                <? if($logged)if($biz->owner == $user->user_id){?>
                                    ...
                                    <div class="pull-left mb5" ><a href="<?=$siteurl;?>/BusinessCP/editWeeklyProgram-<?=$primkey ;?>"><i class="fa fa-pencil"></i></a></div>

                                <?}?>
                                ساعات کاری
                            </div>
                            <div class="row <?=$todayWeekAndHour[0] == 0 ? 'green" rel="tooltip" data-placement="left" title="امروز' : "";?>">
                                <div class="col-md-3 col-xs-3 red ssmall"><?=$day1;?></div>
                                <div class="col-md-4 col-xs-4 ssmall"><?=$program[0]." <span class='hide500'>".toDateName($program[0])."</span>";?> - <?=$program[1]." <span class='hide500'>".toDateName($program[1])."</span>";?></div>
                                <div class="col-md-2 col-xs-2 ssmall">ساعت:</div>
                                <div class="col-md-3 col-xs-3 ssmall">شنبه</div>
                            </div>
                            <div class="row <?=$todayWeekAndHour[0] == 1 ? 'green" rel="tooltip" data-placement="left" title="امروز' : "";?>">
                                <div class="col-md-3 col-xs-3 red ssmall"><?=$day2;?></div>
                                <div class="col-md-4 col-xs-4 ssmall"><?=$program[2]." <span class='hide500'>".toDateName($program[2])."</span>";?> - <?=$program[3]." <span class='hide500'>".toDateName($program[3])."</span>";?></div>
                                <div class="col-md-2 col-xs-2 ssmall">ساعت:</div>
                                <div class="col-md-3 col-xs-3 ssmall">یکشنبه</div>
                            </div>
                            <div class="row <?=$todayWeekAndHour[0] == 2 ? 'green" rel="tooltip" data-placement="left" title="امروز' : "";?>">
                                <div class="col-md-3 col-xs-3 red ssmall"><?=$day3;?></div>
                                <div class="col-md-4 col-xs-4 ssmall"><?=$program[4]." <span class='hide500'>".toDateName($program[4])."</span>";?> - <?=$program[5]." <span class='hide500'>".toDateName($program[5])."</span>";?></div>
                                <div class="col-md-2 col-xs-2 ssmall">ساعت:</div>
                                <div class="col-md-3 col-xs-3 ssmall">دوشنبه</div>
                            </div>
                            <div class="row <?=$todayWeekAndHour[0] == 3 ? 'green" rel="tooltip" data-placement="left" title="امروز' : "";?>">
                                <div class="col-md-3 col-xs-3 red ssmall"><?=$day4;?></div>
                                <div class="col-md-4 col-xs-4 ssmall"><?=$program[6]." <span class='hide500'>".toDateName($program[6])."</span>";?> - <?=$program[7]." <span class='hide500'>".toDateName($program[7])."</span>";?></div>
                                <div class="col-md-2 col-xs-2 ssmall">ساعت:</div>
                                <div class="col-md-3 col-xs-3 ssmall">سه شنبه</div>
                            </div>
                            <div class="row <?=$todayWeekAndHour[0] == 4 ? 'green" rel="tooltip" data-placement="left" title="امروز' : "";?>">
                                <div class="col-md-3 col-xs-3 red ssmall"><?=$day5;?></div>
                                <div class="col-md-4 col-xs-4 ssmall"><?=$program[8]." <span class='hide500'>".toDateName($program[8])."</span>";?> - <?=$program[9]." <span class='hide500'>".toDateName($program[9])."</span>";?></div>
                                <div class="col-md-2 col-xs-2 ssmall">ساعت:</div>
                                <div class="col-md-3 col-xs-3 ssmall">چهارشنبه</div>
                            </div>
                            <div class="row <?=$todayWeekAndHour[0] == 5 ? 'green" rel="tooltip" data-placement="left" title="امروز' : "";?>">
                                <div class="col-md-3 col-xs-3 red ssmall"><?=$day6;?></div>
                                <div class="col-md-4 col-xs-4 ssmall"><?=$program[10]." <span class='hide500'>".toDateName($program[10])."</span>";?> - <?=$program[11]." <span class='hide500'>".toDateName($program[11])."</span>";?></div>
                                <div class="col-md-2 col-xs-2 ssmall">ساعت:</div>
                                <div class="col-md-3 col-xs-3 ssmall">پنج شنبه</div>
                            </div>

                            <div class="row <?=$todayWeekAndHour[0] == 6 ? 'green" rel="tooltip" data-placement="left" title="امروز' : "";?>">
                                <div class="col-md-3 col-xs-3 red ssmall"><?=$day7;?></div>
                                <div class="col-md-4 col-xs-4 ssmall"><?=$program[12]." <span class='hide500'>".toDateName($program[12])."</span>";?> - <?=$program[13]." <span class='hide500'>".toDateName($program[13])."</span>";?></div>
                                <div class="col-md-2 col-xs-2 ssmall">ساعت:</div>
                ...
                if($biz->first_comment !== NULL){
                    ...
                    <div class="mt15 panel panel-default bt-rd4">
                        <div class="panel-body" >
                            اولین نظر :
                            <a href="<?=$siteurl;?>/profile/<?=$biz->first_comment;?>/<?=str_replace(" ", "_",$fcUser->realname);?>" class="mr5 red avgsize">
                    ...
                    <div class="panel-body" style="text-align:justify" >
                        <div class="avgsize red mb10" style="border-bottom:1px dashed #ccc;padding-bottom:10px">
                            <? if($logged)if($biz->owner == $user->user_id){?>
                                ...
                                <div class="pull-left mb5" ><a href="<?=$siteurl;?>/BusinessCP/editBiz-<?=$primkey ;?>"><i class="fa fa-pencil"></i></a></div>
                        ...
                        if(count($userPics) > 0){
                            foreach($userPics as $userPic){?>
                                <div class="<?if($pidId== 1)echo 'active ';;?>item" data-slide-number="<?=$pidId;?>">
                                    <a href="<?=$siteurl;?>/@<?=$primkey;?>/photos/گالری_عکس_<?=str_replace(" ", "_",$biz->name);?>"><img class="rad5" src="http://citygramcdn.ir/bphoto/<?=urlencode(imageEncrypt($userPic['address'].':::16:1'));?>/360.ctg" alt="<?=$biz->name;?>" style="width:360px;height:360px"></a>
                                </div>
                                <?php
                                ++$pidId;
                            }
                            ...
                            <div class="<?if($pidId== 1)echo 'active ';;?>item" data-slide-number="<?=$pidId;?>">
                                <a href="<?=$siteurl;?>/@<?=$primkey;?>/addPic/افزودن_عکس_به_<?=str_replace(" ", "_",$biz->name);?>" ><img class="rad5" src="http://citygramcdn.ir/bphoto/<?=urlencode(imageEncrypt('usrUpload/text.png:" ":::1'));?>/360.ctg" alt="بدون عکس" style="width:360px;height:360px"></a>
                            </div>
                ...
                <div class="mt15 panel panel-default bt-rd4">
                    <div class="panel-body" >
                        <a href="<?=$siteurl;?>/@<?=$primkey;?>/photos/گالری_عکس_<?=str_replace(" ", "_",$biz->name);?>" class="mr5 grey avgsize">
                    ...
                    <div class="mt15 panel panel-default bt-rd4">
                        ...
                        <div class="panel-body newgrams">
                            ...
                            foreach($bussinsses as $bussinss){
                                ...
                                ?>
                                <a href="<?=$siteurl;?>/@<?= $bussinss['primkey'] == "" ? $bussinss['city'].$bussinss['bid'] : $bussinss['primkey'] ;?>/<?=str_replace(" ", "_",$bussinss['name']);?>" title="<?=$bussinss['name'];?>" target="_blank">
                                    <img src="http://citygramcdn.ir/bphoto/<?=urlencode(imageEncrypt($bizPhoto.':" "::20'));?>/18.ctg" class="ml5" alt="<?=$bussinss['name'];?>" style="width:20px;height:20px" /> <?=$bussinss['name'];?>
                                </a>
                            <? } ?>
                        </div>
                    </div>
                ...
                <div class="mt5 mb20">
                    <a href="https://plus.google.com/share?url=<?=$siteurl."/@".$primkey;?>" target="_blank" title="" rel="nofollow"><i class="fa fa-google-plus-square" style="color:#DD4B39;font-size:38px"></i></a>
                    <a href="http://www.facebook.com/sharer.php?u=<?=$siteurl."/@".$primkey;?>" target="_blank" rel="nofollow"><i class="fa fa-facebook-square" style="color:#3B5998;font-size:38px"></i></a>
                    <a href="http://twitter.com/home?status=Currently reading <?=$biz->name . " See at:" . $siteurl."/@".$primkey;?>" target="_blank" rel="nofollow"><i class="fa fa-twitter-square" style="color:#55ACEE;font-size:38px"></i></a>
                    <a href="https://www.linkedin.com/shareArticle?mini=true<?="&title=".$biz->name . "&url=" . $siteurl."/@".$primkey;?>&summary=<?=$biz->address."-".$biz->biz_des;?>&source=citygram.ir" target="_blank" rel="nofollow"><i class="fa fa-linkedin-square" style="color:#3b5998;font-size:38px"></i></a>
                </div>

            </div>

            <style>
                #loader{display:none;position:fixed;right:50%;margin-right:-70px;width:140px;top:50%;padding:8px;border-radius:8px;z-order:1000;z-index:555555555;background:#fff;text-align:center;vertical-align:middle}
                #loader i {display: block;margin-bottom:10px;font-size:50px}
            </style>
            <div id="loader"></div>
            ...
            <div class="col-md-8 col-xs-12">
                <? if($logged)if($biz->owner !== $user->user_id && $letAddComment == 1){?>
                    ...
                    <div class="panel panel-default bt-rd4">
                        <form class="ctgAjaxFrom" data-res="#reveiwBizpage" action="<?=$siteurl;?>/model" method="GET" >
                            <div class="panel-body black" >
                                ...
                                <img src="http://citygramcdn.ir/bphoto/<?=urlencode(imageEncrypt($bizPhoto.':" "::20'));?>/90-90.ctg" class="pull-right" style="margin-left:20px;max-width:90px;width:23%;height:auto" />
                                نظر شما راجع به <span class="red"><?=$biz->name;?></span> چیه ؟<br/>
                                <div class="small">لطفا توضیح خلاصه و کاملی از کسب و کار بنویسید ... </div>
                                <input type="hidden" name="key" value="<?=encryptIt('bizPageAddReveiw_'.random_string(8));?>" />
                                ...
                                if($lastStarAdded == 0){?>
                                    <input id="input-22" value="" name="userReviewRating" type="number" class="rating" data-min="0" data-max="5" data-step="0.5" data-hoverChangeStars="false" data-rtl="1" data-size="xs" data-container-class="ltr mt5 text-right" data-glyphicon="0" >
                                <? } ?>
                                <textarea name="userReveiw" class="mt5" style="width:100%;border-radius:3px;border:1px solid #ccc;"></textarea>
                                <div id="reveiwBizpage"></div>
                            </div>
                            <div class="panel-footer" >
                                <button class="btn btn-danger mt5">ثبت نظر</button>
                                <span class="pull-left" ><a href="<?=$siteurl;?>/guide/review" class="mt5 ml5 ssmall">راهنمای نظردهی</a></span>
                            </div>
                        </form>
                    </div>

                <? } ?>
                <style>.timeline-centered .tooltip{min-width:80px;float:none;text-align:center !important}</style>
                <div class="timeline-centered">
                    ...
                    foreach($timelines as $tmline){
                        ...
                        if($tmline['type'] == "comment"){
                            ...
                            $tmdata = '<img src="http://citygramcdn.ir/bphoto/'.urlencode(imageEncrypt($tmline["data"].":::18:1")).'/300.ctg" alt="'.$biz->name.'" title="'.$biz->name.'" class="img-responsive img-rounded mt5" />'.$letAddComment;
                        ...
                        <article class="timeline-entry left-aligned">
                            <div class="timeline-entry-inner">
                                ...
                                <div class="timeline-label">
                                    <span class="pull-left ssmall" title="<?=jdate('d F Y', $tmline['date']);?>" rel="tooltip"><i class="fa fa-calendar ml5"></i> <?=how_datas_dif($tmline['date']);?></span>
                                    <img src="http://citygramcdn.ir/avatar/<?=urlencode(imageEncrypt($tavatar));?>/20.ctg" class="ml5" style="width:20px;height:20px;border-radius:2px;" />
                                    <h2 style="font-size:15px"><a href="<?=$siteurl;?>/profile/<?=$usr->user_id;?>/<?=str_replace(" ", "_",$usr->realname);?>" title="<?=$usr->realname;?>"><?=$usr->realname;?></a> <span class="small"><?=$tmtext;?></span></h2>
                                    <?=$tmdata !== "" ? "<br/>".$tmdata : "" ;?>
                                </div>
                            </div>
                        </article>
                    ...
                    <article class="timeline-entry left-aligned mt20">
                        <div class="timeline-entry-inner">
                            <div class="timeline-icon" id="bizPageLoadMore" rel="tooltip" title="لــود بیشتر" style="margin:2px -2px 0 0 " >
                    ...
                    <article class="timeline-entry left-aligned begin" style="margin-top:40px">
                        <div class="timeline-entry-inner">
                            <div class="timeline-icon bg-color4" rel="tooltip" title="عضــو شـد" style="margin:2px -2px 0 0 " >
                                <i class="fa fa-user-plus"></i>
                            </div>

                            <div class="timeline-label">
                                <span class="pull-left ssmall" title="<?=jdate('d F Y', strtotime($biz->adddate));?>" rel="tooltip"><i class="fa fa-calendar ml5"></i> <?=how_datas_dif($biz->adddate);?></span>
                                <img src="http://citygramcdn.ir/avatar/<?=urlencode(imageEncrypt($biz->photo));?>/20.ctg" class="ml5" style="width:20px;height:20px;border-radius:2px;" />
    ...
    <!-- Reservation -->
    <div class="modal fade" id="onlineReservation" tabindex="-1" role="dialog" aria-labelledby="onlineReservation" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="ctgAjaxFrom" action="<?=$siteurl;?>/model" method="GET" >
                        <input type="hidden" name="key" value="<?=encryptIt('bizPageReservation_'.random_string(8));?>" />
                        <input type="hidden" name="csrf_token" value="<?=getCSRFToken();?>"/>
                        <input type="hidden" name="bid" value="<?=$biz->bid;?>"/>
                        منوی رزرو :
                        <select name="kindOfReserve" id="kindOfReserve" class="noselect mr10" style="width:20%">
                            ...
                            foreach($reservationMenus as $reservationMenu) {?>
                                <option data-price="<?= $reservationMenu['price'];?>" data-getgrice="<?= $reservationMenu['getPrice'];?>" value="<?= $reservationMenu['rvid'];?>" ><?= $reservationMenu['name'];?></option>
                            <? } ?>
                        </select>
                        تعداد رزرو :
                        ...
                        <hr/>
                        توضیح :
                        <textarea name="reserveDescription" class="mt5" style="width:100%;border-radius:3px;border:1px solid #ccc;"></textarea>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-default" style="float:left;" data-dismiss="modal">بستن پنجره  <i class="fa fa-power-off" style="font-size:13px;"></i></button>
                </div>
            </div>
        </div>
    </div>
